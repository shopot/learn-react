...
- Дочерний компонент ловит этот `ref` и настраивает его через `useImperativeHandle`.

        Внутри него указывается, какие методы или значения будут доступны снаружи.

-  Родитель использует `ref`, но видит только то, что разрешил дочерний компонент.

        Если попытаться вызвать что-то, что не было указано в `useImperativeHandle`, React выдаст ошибку.
